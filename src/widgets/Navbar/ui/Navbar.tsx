...
const Navbar: FC<NavbarProps> = memo(({ className }) => {
  ...
  const[isFormType, setIsFormType] = useState<authConstants.LOGIN | authConstants.REGISTER>(authConstants.REGISTER)
    ...
    <header className={classNames(cls.Navbar, {}, [className])}>
      ...
      <div className={cls.navigation}>
        ...
        <div className={cls.authPanel}>
          {
	          user?.user.email ? <Button onClick={logout}>{t('logout')}</Button> : <>
      ...
      <AuthModal isOpen={isOpenModal} onClose={setIsOpenModal} type={isFormType} />
    </header>
  )
})
